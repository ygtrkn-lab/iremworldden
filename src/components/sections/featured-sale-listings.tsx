...
const FALLBACK_IMAGE = "https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=800&auto=format&fit=crop";
    ...
    const fetchListings = async () => {
      try {
        ...
        const response = await fetch(
          `/api/properties-json?country=TR&type=sale&limit=4&sort=recent`,
        ...
        if (!controller.signal.aborted) {
          console.error("Featured sale listings fetch failed", err);
          setListings([]);
          setError("İlanlar yüklenemedi");
        }
      } finally {
          ...
          {!loading && listings.length === 0 && (
            <div className="col-span-full rounded-3xl border border-dashed border-gray-200 bg-white/70 p-8 text-center text-gray-500">
              <p className="text-sm font-medium">
                {error ?? "Şu anda öne çıkan satılık ilan bulunamadı."}
              </p>
            </div>
          )}
